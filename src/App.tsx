...
export default function App() {
  ...
  useEffect(() => {
    ...
    newSocket.on("scanner-update", (data: ScannerState) => {
      if (data) setScannerState((prev) => ({ ...data, autoTrade: prev.autoTrade, maxConcurrent: prev.maxConcurrent }));
    });
    newSocket.on("scanner:update", (data: ScannerState) => {
      if (data) setScannerState((prev) => ({ ...data, autoTrade: prev.autoTrade, maxConcurrent: prev.maxConcurrent }));
    });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (summaryRes.success && Array.isArray(summaryRes.history)) setHistory(summaryRes.history as TradeHistory[]);
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100 font-sans flex">
      <Sidebar activeTab={activeTab} setActiveTab={setActiveTab} isConnected={isConnected} isBotRunning={runtimeStatus.botRunning} demoTrading={settings.demoTrading} connectionStatus={connectionStatus} />
      ...
      <div className="flex-1 ml-64 p-8 overflow-y-auto h-screen">
        <div className="max-w-6xl mx-auto space-y-8">
          ...
          {activeTab === "terminal" && <TerminalPage balance={balance} isBotRunning={runtimeStatus.botRunning} isCircuitBreaker={runtimeStatus.breakerActive} runtimeStatus={runtimeStatus} isLoading={isLoading} toggleBot={toggleBot} watchlist={watchlist} prices={prices} positions={positions} trades={history} latestKlineUpdate={latestKlineUpdate} logs={logs} onClosePosition={closePosition} onTestOrder={executeTestOrder} />}
          {activeTab === "positions" && <ActiveTradesPage positions={positions} settings={settings} runtimeStatus={runtimeStatus} tradeHistory={history} onClosePosition={closePosition} onRefresh={fetchData} />}
          {activeTab === "strategy" && <StrategyPage watchlist={watchlist} technicals={technicals} toggleWatchlist={toggleWatchlist} trades={history} latestKlineUpdate={latestKlineUpdate} prices={prices} scannerState={scannerState} runtimeStatus={runtimeStatus} activePositions={positions} onToggleAutoTrade={toggleScannerAutoTrade} onScanNow={triggerScanNow} onRefreshMarkets={refreshTopMarkets} onSetMaxConcurrent={setScannerMaxConcurrent} onQuickBuy={executeTestOrder} />}
          {activeTab === "history" && <HistoryPage history={history} />}
          {activeTab === "settings" && <SettingsPage settings={settings} runtimeStatus={runtimeStatus} updateSetting={updateSetting} systemLogs={logs} />}
        </div>
      </div>
    </div>
  );
