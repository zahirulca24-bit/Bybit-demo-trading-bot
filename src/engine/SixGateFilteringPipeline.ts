...
export class SixGateFilteringPipeline {
  public state: PipelineState = {
    ...
    gates: [
      ...
      },
      {
        ...
        ruleDescription: "Confirmed 5m candle RSI 50-65 for Long, 35-50 for Short",
        status: "Active",
        inputCount: 0,
        passCount: 0,
        passRatePercent: 100,
  ...
  public async executePipelineScan(): Promise<PipelineState> {
    ...
    try {
      ...
      for (let i = 0; i < validPairs.length; i += chunkSize) {
        const batch = validPairs.slice(i, i + chunkSize);
        const results = await Promise.all(
          batch.map((ticker: any) => this.evaluateSymbolThroughGates(ticker))
        );
      ...
      this.state = {
        totalDiscovered: validPairs.length,
        passedAllCount: g6Passed.length,
        activeSignalsCount: g6Passed.length,
        gates: gateSummaries,
        symbols: scannedSymbols.slice(0, 20),
  ...
  private summary(
    ...
    inputCount: number,
    passCount: number,
    forcedStatus?: "Passed" | "Filtering" | "Active"
  ): PipelineGateSummary {
    return {
      gateNumber,
      name,
      ...
      status:
        forcedStatus ||
        (inputCount > 0 && passCount === inputCount ? "Passed" : "Filtering"),
      inputCount,
      passCount,
      passRatePercent:
        inputCount > 0 ? Math.round((passCount / inputCount) * 100) : 100,
    };
  }

  private async evaluateSymbolThroughGates(
    ticker: any
  ): Promise<PipelineScannedSymbol | null> {
    try {
      ...
      const [kline15mRes, kline5mRes, oiMetric] = await Promise.all([
        this.bybit
          .getKline({ category: "linear", symbol, interval: "15", limit: 210 })
          .catch(() => null),
        this.bybit
          .getKline({ category: "linear", symbol, interval: "5", limit: 80 })
          .catch(() => null),
      ...
      if (
        !kline15mRes ||
        kline15mRes.retCode !== 0 ||
        !kline15mRes.result?.list ||
        !kline5mRes ||
        kline5mRes.retCode !== 0 ||
        !kline5mRes.result?.list
      ) {
        return null;
      }
      ...
      const spreadPercent = hasRealBook && midPrice > 0
      ...
      if (!isVolumeValid) {
        failedGateNumber = 1;
        failedGateName = "Gate 1: Low 24h Turnover (<$10M)";
      } else if (!isTrend15mValid) {
        failedGateNumber = 2;
        failedGateName = "Gate 2: 15m HTF Trend Neutral / Choppy";
      } else if (!isSpreadValid) {
        failedGateNumber = 3;
        failedGateName = "Gate 3: Missing/Wide Bid-Ask Spread (>0.15%)";
      } else if (!isAtrValid) {
        failedGateNumber = 4;
        failedGateName = "Gate 4: Low Confirmed 5m Volatility (<0.30% ATR)";
      } else if (!isOiValid) {
        failedGateNumber = 5;
        failedGateName = "Gate 5: Real 1h Open Interest Delta Negative/Unavailable";
      } else if (!isRsi5mValid) {
        failedGateNumber = 6;
        failedGateName = "Gate 6: Confirmed 5m RSI Entry Trigger Out of Range";
      }
      ...
      if (passedAll && trend15m === "Bullish HTF") {
        signalAction = "Grade A Long";
        actionType = "LONG";
      } else if (passedAll && trend15m === "Bearish HTF") {
      ...
      const gateResults: GateResultSummary = {
        gate1_volume: {
          ...
          detail: isVolumeValid ? "High 24h Volume Liquidity" : "Below $10M minimum threshold",
        },
        gate2_trend: {
          ...
          detail:
            trend15m === "Bullish HTF"
              ? "Confirmed 15m EMA 50 > EMA 200"
              : trend15m === "Bearish HTF"
              ? "Confirmed 15m EMA 50 < EMA 200"
              : "No confirmed HTF structure",
        },
        gate3_spread: {
          passed: isSpreadValid,
          valueDisplay: Number.isFinite(spreadPercent) ? `${spreadPercent.toFixed(3)}%` : "Unavailable",
          detail: isSpreadValid ? "Fresh tight book (<=0.15%)" : "Fresh spread unavailable or too wide",
        },
        gate4_atr: {
          passed: isAtrValid,
          valueDisplay: `${atr5mPercent.toFixed(2)}%`,
          detail: isAtrValid ? "Confirmed 5m ATR >= 0.30%" : "Confirmed 5m ATR below 0.30%",
        },
        gate5_oi: {
          passed: isOiValid,
          valueDisplay: Number.isFinite(oiMetric.oiChange1h)
            ? `${oiMetric.oiChange1h >= 0 ? "+" : ""}${oiMetric.oiChange1h.toFixed(2)}%`
            : "Unavailable",
          detail: isOiValid ? "Real Bybit OI flat/rising over 1h" : "Real OI falling or unavailable",
        },
        gate6_rsi: {
          passed: isRsi5mValid,
          valueDisplay: `${currentRsi5m.toFixed(1)} (${rsiZone5m})`,
          detail: isRsi5mValid ? "Confirmed 5m candle in entry zone" : "Confirmed 5m RSI outside entry zone",
        },
        passedAll,
        failedGateNumber,
        failedGateName,
      ...
      return {
        ...
        bidPrice,
        askPrice,
        spreadPercent: Number.isFinite(spreadPercent) ? spreadPercent : 999,
        isSpreadValid,
        atr5m: currentAtr,
        atr5mPercent,
        isAtrValid,
      ...
      console.error("[Pipeline] Symbol evaluation error:", err?.message || err);
      return null;
    }
  }
  ...
  private closedCandles(list: any[], intervalMs: number): any[] {
    ...
    return [...list]
      .reverse()
      .filter((c: any) => {
        const openTime = Number(c?.[0] || 0);
        return openTime > 0 && openTime + intervalMs <= now;
      });
  }
  ...
  private async getOpenInterestMetrics(symbol: string): Promise<OiMetric> {
    ...
    try {
      const client = this.bybit as any;
      if (typeof client.getOpenInterest !== "function") {
        return { oi: 0, oiChange1h: Number.NaN, timestamp: Date.now() };
      }

      const response = await client.getOpenInterest({
        category: "linear",
        symbol,
        intervalTime: "1h",
        limit: 3,
      ...
      const ordered = [...response.result.list].sort(
      ...
      if (!latest || !previous) {
        return { oi: Number(latest?.openInterest || 0), oiChange1h: Number.NaN, timestamp: Date.now() };
      }

      const latestOi = Number(latest.openInterest || 0);
      const previousOi = Number(previous.openInterest || 0);
      const oiChange1h = previousOi > 0 ? ((latestOi - previousOi) / previousOi) * 100 : Number.NaN;

      const metric: OiMetric = {
        oi: latestOi,
        oiChange1h,
        timestamp: Date.now(),
      };
      ...
      console.error(`[Pipeline] OI fetch failed for ${symbol}:`, err?.message || err);
      return { oi: 0, oiChange1h: Number.NaN, timestamp: Date.now() };
    }
  }

  private formatTurnover(val?: number): string {
    if (!val || val === 0 || Number.isNaN(val)) return "$0";
